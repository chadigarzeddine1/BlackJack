...
public class TempGUI {
	...
	public Image getcard(Player p,int n) {
	 ...
	 public int setPlayer() {
		   ...
		   Scene scene2 = new Scene(bp,1500,1000, Color.DARKGREEN);
...
//	 }
	 ...
	 public Scene makeBaseScene() {
		 Group root = new Group();	
		 Deck deckd = new Deck();
		 	Card card = deckd.draw();
		   Player player = new Player("boi");
		   player.addCardToHand(card);
		   

	   Button start = new Button(" Start");
	   BorderPane bp = getBordPane();
	   ...
	   Label pc1 = makeLabel(makecard(player,0));
	   Scene scene = getScene();
	   
	   	 
	   Label pc2 = makeLabel(makecard(player,0));
	   Label deck = makeLabel(makecard(player,0));
	   Label dc1 = makeLabel(makecard(player,0));
	   Label dc2 = makeLabel(makecard(player,0));
	 ...
	 public void starts() {
	   ...
	   Button start = new Button(" Starts");
	   BorderPane bp = getBordPane();
	   GridPane gt = new GridPane();
	   GridPane gb = new GridPane();
	   GridPane gl = new GridPane();
	   ...
	   start.setOnAction(new EventHandler<ActionEvent>()   {
		   	@Override
		   	public void handle(ActionEvent event){
		   		primaryStage.setScene(makeBaseScene());
		   	}
		   });
	   setgrid(gtext);
	   setgrid(gr);
